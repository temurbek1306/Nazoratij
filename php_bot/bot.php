...
$GITHUB_REPO = "temurbek1306/Nazoratij";
...
if (isset($update['message'])) {
    ...
    if (isset($update['message']['video']) || isset($update['message']['document']) || isset($update['message']['photo'])) {
        ...
        if ($is_photo && !$is_multiple) {
            sendMessage($chat_id, "⚠️ Rasm faqat 'Videolarni Birlashtirish' rejimida (Thumbnail sifatida) qabul qilinadi.");
            exit;
        }
        
        if ($is_photo) {
            $photos = $update['message']['photo'];
            $file_obj = $photos[count($photos) - 1]; // get largest photo
            $file_size = isset($file_obj['file_size']) ? $file_obj['file_size'] : 0;
        ...
        if ($file_size > 20000000) {
            sendMessage($chat_id, "❌ <b>Xatolik! Fayl hajmi juda katta.</b>\n\nTelegram Bot API faqat <b>20 MB</b> gacha bo'lgan fayllarni qabul qila oladi.");
            exit;
        }
        ...
        if ($file_path) {
            ...
            if (!$is_multiple) {
                ...
                $ext = $is_photo ? '.jpg' : '.mp4';
                $safe_name = 'tg_' . time() . '_' . rand(1000, 9999) . $ext;
                $dest_path = $upload_dir . '/' . $safe_name;
                
                $file_data = file_get_contents($telegram_url);
                if ($file_data && strlen($file_data) > 1000) {
            ...
            if ($is_multiple) {
                ...
                if ($is_photo) {
                    $current_group .= "PHOTO:" . $telegram_url . "\n";
                } else {
                ...
                $type_name = $is_photo ? "Rasm (Thumbnail)" : "Video";
                sendMessage($chat_id, "✅ $count-fayl ($type_name) qabul qilindi. Yana yuboring yoki pastdagi '✅ Birlashtirishni boshlash' tugmasini bosing.");
                exit;
            }
        ...
        if (file_exists(__DIR__ . "/scheduled.json")) {
            ...
            if (is_array($sched) && count($sched) > 0) {
                foreach ($sched as $i => $sv) {
                    ...
                    $url_short = !empty($sv['video_url']) ? substr($sv['video_url'], 0, 40) . "..." : (!empty($sv['artifact_run_id']) ? "Artifact: " . $sv['artifact_run_id'] : "YO'Q");
